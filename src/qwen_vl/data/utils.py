...
def load_and_preprocess_images(image_path_list, mode="crop", target_size=518):
    ...
    for image_path in image_path_list:
        ...
        target_height_limit = 392
        # Center crop height if it's larger than 392 (only in crop mode)
        if mode == "crop" and new_height > target_height_limit:
            start_y = (new_height - target_height_limit) // 2
            img = img[:, start_y : start_y + target_height_limit, :]
