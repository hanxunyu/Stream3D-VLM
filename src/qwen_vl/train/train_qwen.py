...
transformers.trainer.Trainer.store_flos = lambda self: None
...
class ClearCacheCallback(transformers.TrainerCallback):
    ...
    if not _is_qwen2_5:
        ...
        if os.path.exists(_config_path):
            ...
            with open(_config_path, "r") as _f:
                _cfg = _json.load(_f)
            _is_qwen2_5 = _cfg.get("model_type", "").lower() in ("qwen2_5_vl", "qwen2.5_vl")
    if _is_qwen2_5:
        if not model_args.use_geometry_encoder:
            ...
            for k in [
                ...
                "geometry_encoder_train_or_eval_mode"
            ]:
            ...
            frame_interval_token_id = temp_tokenizer.encode(',', add_special_tokens=False)[0]
            frame_end_token_id = temp_tokenizer.encode('\n', add_special_tokens=False)[0]
            print(f"Loaded special tokens: ',' ID = {frame_interval_token_id}, '\\n' ID = {frame_end_token_id} in train_qwen.py")
            
            config.frame_interval_token_id = frame_interval_token_id
            config.frame_end_token_id = frame_end_token_id
            config.stream_loss_weight = getattr(model_args, 'stream_loss_weight', 2.0)
            config.llm_loss_weight = getattr(model_args, 'llm_loss_weight', 1.0)

            
            if local_rank == 0:
                print(f"Frame interval token ',' ID: {frame_interval_token_id}")
                print(f"Frame end token '\\n' ID: {frame_end_token_id}")
                print(f"Stream loss weight: {config.stream_loss_weight}")
                print(f"LLM loss weight: {config.llm_loss_weight}")
