...
class ModelArguments:
    ...
    geometry_encoder_train_or_eval_mode: str = field(default="eval")  # Mode for geometry encoder ("train" or "eval")
    ...
    # Frame-decision related parameters
    stream_loss_weight: float = field(default=2.0, metadata={"help": "Weight for stream loss (frame decision)"})
    llm_loss_weight: float = field(default=1.0, metadata={"help": "Weight for LLM loss (language generation)"})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # Avoid triggering NCCL OOM during save / log steps.
    skip_memory_metrics: bool = field(
        default=True,
        metadata={"help": "Skip GPU memory/FLOPs metrics to avoid NCCL OOM in distributed training."}
    )
